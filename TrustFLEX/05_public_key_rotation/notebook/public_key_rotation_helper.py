...
def get_rotating_key():
    with open(os.path.join(path, "TrustFLEX/00_resource_generation", ROTATING_KEY), 'rb') as f:
...
def get_validating_authority_key():
    with open(os.path.join(path, "TrustFLEX/00_resource_generation", VALIDATION_AUTHORITY), 'rb') as f:
        return serialization.load_pem_private_key(
                data=f.read(),
                password=None,
                backend=default_backend())
...
def generate_new_rotate_public_key_pair():
        ...
        with open(os.path.join(path, "TrustFLEX/00_resource_generation", ROTATING_KEY), 'wb') as f:
