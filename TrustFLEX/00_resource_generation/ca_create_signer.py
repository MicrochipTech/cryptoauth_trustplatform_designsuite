...
def ca_create_signer(
    ...
    not_before = datetime.utcnow().replace(tzinfo=timezone.utc, minute=0, second=0)
    ...
    with open('signer-ca.cer', 'wb') as f:
        f.write(signer_ca_cert.public_bytes(encoding=serialization.Encoding.PEM))

    return signer_ca_cert.public_bytes(encoding=serialization.Encoding.DER)
    #print('\nDone')
