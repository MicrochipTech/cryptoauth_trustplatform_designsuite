...
def mchp_cert_bckp():
    ...
    if(mchp_cert_info['status'] == Status.ATCA_SUCCESS):
        ...
        # Check for backups
        if(os.path.exists(os.path.join('mchp_certs_bckp', device_file_name))):
            ...
            signer_cert_def, device_cert_def = certs_handler.generate_cert_def_files(root_cert_path, 'FFFF', signer_cert_path, device_cert_path)

            print('Saving signer certificate to device...', end='')
            signer_cert_der = read_cert(signer_cert).public_bytes(encoding=Encoding.DER)
            ...
            device_cert_der = read_cert(device_cert).public_bytes(encoding=Encoding.DER)
