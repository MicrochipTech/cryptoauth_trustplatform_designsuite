...
def ca_create_root(root_ca_key_path=ROOT_CA_KEY_FILENAME, root_ca_cert_path=ROOT_CA_CERT_FILENAME, root_org_name=u'Microchip Technology Inc'):
    # Create or load a root CA key pair
    print('Loading root CA key')
    root_ca_priv_key = load_or_create_key(root_ca_key_path, verbose=False)
    ...
    builder = builder.issuer_name(x509.Name([
        ...
        x509.NameAttribute(x509.oid.NameOID.COMMON_NAME, u'Crypto Authentication Root CA 000')]))
    ...
    return root_ca_priv_key.public_key().public_numbers().encode_point()[1:]
    #print('\nDone')
