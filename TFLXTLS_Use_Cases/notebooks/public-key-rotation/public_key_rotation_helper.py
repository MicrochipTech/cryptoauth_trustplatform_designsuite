...
def get_rotating_key():
    with open(os.path.join(path, "TFLXTLS_resource_generation", ROTATING_KEY), 'rb') as f:
...
def get_validating_authority_key():
    with open(os.path.join(path, "TFLXTLS_resource_generation", VALIDATION_AUTHORITY), 'rb') as f:
        return serialization.load_pem_private_key(
                data=f.read(),
                password=None,
                backend=default_backend())
...
def resource_generate():
    ...
    with open(os.path.join(path, "TFLXTLS_resource_generation", application_h), 'w') as f:
    ...
    assert validate_status == True
    with open(os.path.join(path, "TFLXTLS_resource_generation", application_h), 'a') as f:
...
def generate_new_rotate_public_key_pair():
        ...
        with open(os.path.join(path, "TFLXTLS_resource_generation", ROTATING_KEY), 'wb') as f:
...
def get_rotating_public_key():
    rotating_private_key = get_rotating_key()
    return rotating_private_key.public_key().public_bytes(encoding=Encoding.X962, format=PublicFormat.UncompressedPoint)[1:]
