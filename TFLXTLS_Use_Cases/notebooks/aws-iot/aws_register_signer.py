...
def aws_register_signer(
    ...
    if not os.path.isfile(os.path.join(path, "TFLXTLS_resource_generation", SIGNER_CA_KEY_FILENAME)):
        raise AWSZTKitError('Failed to find signer CA key file, ' + SIGNER_CA_KEY_FILENAME + '. Have you run ca_create_signer_csr first?')
    with open(os.path.join(path, "TFLXTLS_resource_generation", SIGNER_CA_KEY_FILENAME), 'rb') as f:
    ...
    if not os.path.isfile(os.path.join(path, "TFLXTLS_resource_generation", SIGNER_CA_CERT_FILENAME)):
    ...
    with open(os.path.join(path, "TFLXTLS_resource_generation", SIGNER_CA_CERT_FILENAME), 'rb') as f:
    ...
    with open(os.path.join(path, "TFLXTLS_resource_generation", SIGNER_CA_VER_CERT_FILENAME), 'wb') as f:
