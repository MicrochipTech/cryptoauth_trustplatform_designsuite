...
def read_kit_info():
    if not os.path.isfile(os.path.join(path, "TrustFLEX/00_resource_generation", KIT_INFO_FILENAME)):
        return {}
    with open(os.path.join(path, "TrustFLEX/00_resource_generation", KIT_INFO_FILENAME), 'r') as f:
        json_str = f.read()
        if not json_str:
            return {}  # Empty file
...
def save_kit_info(kit_info):
    with open(os.path.join(path, "TrustFLEX/00_resource_generation", KIT_INFO_FILENAME), 'w') as f:
